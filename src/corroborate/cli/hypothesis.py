"""CLI surface for `corroborate.runner.run`.
...
- `corroborate hypothesis ...` (subcommand under the main CLI;
  wired via `add_args` + `dispatch` from `corroborate.__main__`).
- `python -m corroborate hypothesis ...` (same).
- `python scripts/run_hypothesis.py ...` (back-compat shim).

Library code lives in `corroborate.runner`; this file is purely
the argparse + verdict-printing surface."""
...
def add_args(parser: argparse.ArgumentParser) -> None:
    """Register hypothesis-subcommand arguments onto `parser`.

    Mirrored by `dispatch(args)`, which acts on the parsed
    Namespace. Two callers:
    - `corroborate.__main__._build_parser()` for the `corroborate
      hypothesis ...` subparser; and
    - `main()` below for `python -m corroborate.cli.hypothesis` /
      the `scripts/run_hypothesis.py` shim.
    """
    parser.add_argument(
        'module',
...
def dispatch(args: argparse.Namespace) -> int:
    """Run the hypothesis subcommand body against parsed `args`.

    Mirror of `add_args`: the top-level CLI parses with the shared
    arg set, then calls into `dispatch`. Standalone-script
    invocation goes through `main()` which builds a one-shot parser
    and then delegates here.
    """
    # The hypothesis arg is a dotted module path (e.g.
    # `experiments.findings.ddqn`) that the user typically rooted at
    # CWD. The console-script entry point doesn't put CWD on
    # `sys.path` (unlike `python scripts/run_hypothesis.py`, where
    # CPython adds the script's directory). Make the two surfaces
    # behave the same by prepending CWD — matches the convention
    # pytest / mypy / ruff use for user-module loading.
    import sys
    cwd = str(Path.cwd())
...
def main(argv: Sequence[str] | None = None) -> int:
    """Standalone entry point for `python -m corroborate.cli.hypothesis`
    or for the `scripts/run_hypothesis.py` back-compat shim.

    Builds a one-shot ArgumentParser, registers args via `add_args`,
    and delegates to `dispatch`. The top-level `corroborate` CLI
    wires the same `add_args`/`dispatch` pair as a subparser — both
