...
Mediation-aware companion to `stratum_outcome_attenuation_dowhy`:
that primitive tests whether a binary attenuator moves Δ_target
directly; this primitive tests whether it moves the SLOPE of
Δ_target on Δ_predictor (the link). When the science is a
mediation claim ("extreme Q-divergence attenuates the
bias→outcome link"), the slope-moderation form is the right
shape — Δ_target alone can't see the link's existence.
