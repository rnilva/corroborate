...
Downstream consumers (`dl_pool`, `fisher_z_pool`, `panel_partial`,
`panel_regress`, `scope_from_panel`, the `stratum_*_dowhy` family)
take this panel as a fixture (resolved by parameter injection per
the @analysis fixture pattern) and produce specialized aggregates.
...
unification targets the cross-arm family: `dl_pool`,
`fisher_z_pool` on cross-arm pairs, `panel_partial` on cross-arm
partials, plus the `stratum_*_dowhy` family."""
