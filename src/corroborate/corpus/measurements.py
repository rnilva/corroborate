...
def resolve_runs_meas_collision(
    *,
    ...
    - Registered measurable AND `transitive_reads ⊆ runs_cols`
      → runs wins (substrate had inputs at sweep time + the
      stamp is authoritative). Drop from meas.
    - Registered measurable AND reads NOT in `runs_cols`
      → meas wins (trace-dependent; runs.parquet carries only a
      sweep-time NaN stamp; the measurements file was
      post-sweep computed when traces were available). Drop
      from runs.
    - Unregistered (or `transitive_reads` raises) →
      `unregistered_policy` decides:
        * `'runs_wins'` (build_measurements default):
          defensive CI6 fallback; the runner has the registry
          import, non-registered overlap shouldn't happen in
          practice.
    ...
    Single source of truth — both `build_measurements` and
    `data.panel.Panel.from_corpus` delegate here so collision
    semantics can't drift across surfaces."""
    from corroborate.measurables.measurable import transitive_reads
    ...
    for c in meas_cols & runs_cols:
        ...
        if all(r in runs_cols for r in leaf_reads):
            drop_from_meas.add(c)  # runs wins (substrate could compute)
        else:
            drop_from_runs.add(c)  # meas wins (trace-dependent)
