...
def _per_group_stats(
    ...
    *,
    ...
    """Pair within one group, compute Hedges' g + raw stats →
...
def hypothesis_comparison_from_cells(
    ...
    *,
    ...
    `predicted_direction` is recorded on the row so downstream
    bridge-evaluations can apply their own sign tests against the
    edge's prior. The aggregator itself no longer derives a
    verdict — bridges author the threshold logic via `holds_when`.
