...
def main() -> None:
    ...
    caveats = (
        ...
        '"Bias mediates beyond MC-leak" is consistent with TWO\n'
        'non-exclusive stories:\n\n'
        '  (a) Bias-clip mechanism (Hasselt 2010\'s claim) —\n'
        '      DDQN\'s clip directly reduces bias, which proximately\n'
        '      causes outcome improvement.\n\n'
        '  (b) Q-via-state-visitation — DDQN changes Q; Q changes\n'
        '      the policy; the policy changes state visitation;\n'
        '      state visitation changes outcome. Bias contains Q,\n'
        '      so it predicts outcome via this longer pathway.\n\n'
        'The test cannot distinguish (a) from (b).\n\n'
        'Next falsification step: add the panel\'s state-visitation\n'
        'measurables as multi-input siblings\n'
        '(state_hash_n_unique_per_burst,\n'
        ' state_hash_entropy_per_burst,\n'
        ' state_repeat_rate_window64_per_burst).\n'
        'If conditioning on state visitation ALSO absorbs bias\'s\n'
        'mediation power → (b) is more parsimonious.\n'
        'If not → (a) survives.'
    )
