...
RL's verdicts on its own algorithms — modest, mixed, ablation-conditional — are downstream of a single collapse: scalar summaries cannot distinguish where a theorem's premise admits from where the chain to outcome breaks. Recovery is structural: the mechanism enters as a first-class upstream edge of the causal graph, testable independently of the outcome edge.

We present `corroborate`, a framework that types mechanism claims as falsifiable Python programs — falsifiable not only against future data but against the authors' own subsequent analyses, as multiple in-flight walk-backs during the case study attest. The theorem's premise and its predicted empirical signature become typed measurables; the edges between them are the claims under test, and premise non-activation is the failure of an edge, not a noisy null at outcome. Verdicts compose under a power-aware trichotomy (HELD / REFUTED / POWER_INSUFFICIENT) that propagates underdetermination through composed claims rather than silently collapsing it. Where per-paper methodologies are non-comparable, the framework provides a shared substrate — registered analyses, typed measurables, common verdict semantics — making per-claim findings commensurable.

Double-DQN is the exemplar. Across ten environments and intervention sweeps, the case study replaces the modest-or-universal verdict with a structural account. The Jensen-bias floor (Hasselt 2010) functions empirically as the premise indicator: per-cell premise-activation types each environment as where the bound bites or where it is dormant, and DDQN's bias-reduction mechanism corroborates the theorem exactly where it should. The bias-correction→outcome link is genuinely env-conditional, scoped by the bootstrap fraction; goal-reaching and survival environments exhibit opposite mediator signs under the same intervention. In sparse-reward regimes DDQN's outcome direction sign-aligns with a within-episode loop-reduction channel — pointing to mediator candidates beyond the canonical bias-clip path.
...
- **v12 (substantive position correction + flow pass)**: P1 rewritten — startling-fact opening "A theorem's premise can be measured per cell" replaced with domain-anchored collapse-diagnostic "RL's verdicts on its own algorithms... are downstream of a single collapse: scalar summaries cannot distinguish where a theorem's premise admits from where the chain to outcome breaks." S2 corrects the position: the *where does the mechanism fire?* framing in v11 reflected an old per-cell-premise-filtering approach the framework retreated from (post-treatment / M-bias trap when the intervention itself shifts which cells satisfy the premise). The true current position: "the mechanism enters as a first-class upstream edge of the causal graph, testable independently of the outcome edge" — Pearl-style mediation with the mech edge separable from the outcome edge, NOT cell-level localization of where the theorem applies. P2 follows: `operationalises per-cell premise verification by typing mechanism claims` → `types mechanism claims` (drops the same old framing). "Theoretical bounds enter as first-class upstream edges in a typed causal graph" → "The theorem's premise and its predicted empirical signature become typed measurables; the edges between them are the claims under test" (more specific about what's typed and what's tested). Other flow: `codifying` → `typing`; `through chains` → `through composed claims`; dropped `the field's` and `structurally` as implied; `common measurables` → `typed measurables`; cut "As one concrete curiosity the framework surfaces," from P3 closing — implicit.
