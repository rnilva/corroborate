...
| 12 | `ddqn_refuted_when_dormancy_fires` | POWER_INSUFFICIENT | (2026-05-11 migrated `paired_g + bootstrap_paired_g` → `stratified_arm_diff_pooled` per RL seed-pairing critique + dormancy threshold tightened 1e-9→0.05 + per-env CI logic (not pooled-only). Per-env at threshold 0.05: Acrobot d=+1.0 wide CI, Breakout ~0, SI d=+0.49 CI=[+0.13, +0.85]. **KNOWN MEASUREMENT-FRAME ARTIFACT**: `jensen_dormancy_gap` collapses all bursts to one scalar; `eval_best_burst_mean` picks the best burst. Different aggregation windows — same misalignment issue as CLAIM 26b authoring. Per-env effects (SI d=+0.49) may be artifacts of dormancy-at-pooled-bursts vs outcome-at-best-burst. Verdict POW_INSUF honestly reflects "can't tell — measurement frame misaligned". TODO: author `jensen_dormancy_gap_per_burst` + use `mc_return_per_burst_mean` for outcome at same window) | null | 315 | 1824 |
