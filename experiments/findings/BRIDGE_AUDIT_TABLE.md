...
| 12 | `ddqn_refuted_when_dormancy_fires` | POWER_INSUFFICIENT | (2026-05-11 migrated `paired_g + bootstrap_paired_g` → `stratified_arm_diff_pooled` per RL seed-pairing critique: trajectories diverge so seed-pairing isn't a real pair. Per-env Cohen's d (independent-samples, Hedges 1981 SE) + DerSimonian-Laird pooling. Pooled d=+0.068, CI=[−0.288, +0.423] spans ±0.2 null_ceiling on both sides; I²=0.71 (high cross-env heterogeneity — Acrobot d=+0.57, Asterix d=−0.68, Breakout ~0, Freeway +0.17, SI +0.45). The necessary-condition null isn't confirmed at the cross-env aggregate) | null | 557 | 1824 |
