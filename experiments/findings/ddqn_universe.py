...
Bridges encoding the DDQN causal-chain claims. The current
verdict landscape lives in `experiments/findings/ddqn_universe.run.json`
(regenerated by `scripts/run_hypothesis.py`); see
`BRIDGE_AUDIT_TABLE.md` for the audit history.
...
## Two-channel causal architecture

The post-2026-05-12 reading: DDQN helps outcome via TWO distinct
channels with different sufficient conditions:

  Channel A — Hasselt bias-correction. Bridge fires when
              `[G1 ∧ G2 ∧ G3]` (premise-active ∧
              argmax-vulnerable ∧ outcome-headroom). DDQN
              reduces vanilla's Q-overestimation; the bias
              reduction propagates to outcome through the
              link mechanism. Tested by CLAIM 26b.

  Channel B — Q-magnitude regularization. Bridge fires when
              dormant (G1 inactive) but the algorithmic clip
              wedge `ddqn_bootstrap_gap = target_max_q −
              target_q[argmax_online]` is substantively active.
              DDQN's bootstrap pulls Q-magnitude toward
              less-positive direction even when there's no
              Hasselt bias to correct — a cross-seed stability
              regularizer. Tested by CLAIM 3.

These are NOT competing hypotheses — they're parallel channels
on disjoint scopes. The framework's CLAIM 2 (necessary-condition)
remains POWER_INSUFFICIENT because Acrobot + SI dormant cells
show DDQN benefit via Channel B even when Channel A is dormant.

## Surviving load-bearing bridges

- **Mechanism activation** (universal, Channel A) — `do(arm=ddqn)
...
- **CLAIM 2 — Necessary scope (dormancy refutation,
  POWER_INSUFFICIENT)**. `ddqn_refuted_when_dormancy_fires`
  migrated to `stratified_arm_diff_pooled` 2026-05-11 (per-env
  Cohen's d + DerSimonian-Laird pooling, independent-samples
  form — no seed-pairing). Empirical: Acrobot d=+0.43 CI=[0.06,
  0.79] and SI d=+0.37 CI=[0.03, 0.72] BOTH span the +0.2
  null-ceiling. Verdict POW_INSUF reflects "small-substantive
  non-null" — the necessary-condition framing is leaning
  refuted, awaiting CLAIM 3's mechanism explanation (Channel B
  fires on these very cells).
- **CLAIM 3 — Q-clip channel sufficient (Channel B)**.
  `clip_wedge_predicts_outcome__dormant_scope` (added
  2026-05-12). Stratified partial Spearman `ρ(clip_wedge,
  outcome | jens)` on truly-dormant cells (`jensen_gap < 0.1`),
  env-stratified, Fisher-z pooled. Tests whether the Q-clip
  channel explains the residual DDQN benefit CLAIM 2 cannot
  account for via Hasselt bias-correction.
- **CLAIM 26b — Three-gate scope conjunction (Channel A
  sufficient)**. `ddqn_helps_under_three_gate_scope__cross_env`.
  DDQN helps iff `G1 ∧ G2 ∧ G3` fires jointly. Pooled d=+0.36,
  p=0.007 across 7 G1-active configs (DL random-effects, fine-
  stratified by env × sync × γ × T post-2026-05-11 refinement).
  The substantive cross-config outcome-level claim for Channel A.
- **CLAIM 22 — REACH-cohort link (DoWhy)**. Backdoor + placebo
  + RCC refutation trio on the REACH-polarity envs. The
  Pearl-rung-3 corroboration that `Δ_jens → Δ_outcome` is
  causally identified on the REACH subset.
- **CLAIM 17 (chain-amplifier link) — CUT 2026-05-11**.
  Migrated to `panel_regress` revealed cross-env signal was
  leverage-driven. Substance preserved by CLAIM 26b's
  stratified-DL pool (leverage-robust) and per-env existence
  proofs in `findings_minatar_link_attenuation.md`.

## Methodology note: seed-pairing critique

Inside per-pair-Δ fixtures (`paired_g`, `paired_link_per_burst`,
`proportion_mediated`), seed-pairing reflects within-init
correlation rather than population-of-inits variance — the
inferential target for cross-init claims is the
stratified / independent-samples / partial-Spearman form. The
2026-05-11/12 audit migrated the load-bearing bridges to:

- `stratified_arm_diff_pooled` (DerSimonian-Laird Cohen's d
  pooling, independent-samples Hedges form)
- `arm_mean_diff` (Welch's t, true independent samples)
- `stratified_partial_spearman` (JCI per-cell conditional
  independence, env-stratified)
- `within_arm_link` (per-arm Pearson r, no pairing)

`paired_g` + `paired_link_per_burst` + `proportion_mediated`
are retained for non-stochastic-env analyses (e.g., LG-SCM
analytic tests) but should NOT be used as the primary
inference engine on RL panels with stochastic envs."""
...
# =====================================================================
# RESTORED 2026-05-12 (audit reflection): adaptive_dqn_recovers__
# fourrooms_factor_0p5 is the Pearl rung-2 dog-fooding of the
# framework's own dormancy invariant. A runtime controller built
# FROM the framework's per-batch dormancy proxy recovers DDQN's
# outcome benefit on FourRooms (HELD g=+0.78, p<0.001 historically).
# This is the most probative evidence the framework produces —
# verdict-level corroboration that the dormancy invariant carries
# real predictive power, not just descriptive labeling. Memory note
# `findings_dormancy_controller_scope.md` records the empirical
# numbers but is not equivalent to the framework's verdict.
#
# Currently AWAITING DATA: `adaptive_dqn_fourrooms_sweep` +
# `expectile_3way` corpora are absent from the postfix cache.
# Bridge returns POWER_INSUFFICIENT until adaptive sweeps return.
#
# Methodology note: bridge still consumes paired_g. The per-pair-Δ
# critique applies; future migration to arm_mean_diff is deferred
# until data returns (so the migration's empirical effect can be
# verified against fresh cells).
# =====================================================================
...
def adaptive_dqn_recovers_ddqn_benefit__fourrooms_factor_0p5(
    ...
    `max_greedify` and DDQN `double_greedify` based on the in-batch
    dormancy proxy `max_Q − mean_Q ≥ 0.5 × σ_Q × √(2 log |A|)`.

    The claim: a runtime controller built FROM the framework's
    own dormancy invariant recovers DDQN's outcome benefit on
    FourRooms — Pearl rung-2 corroboration of CLAIM 2's necessary-
    condition reading.

    HELD when g(adaptive − vanilla) ≥ +0.50 AND p < 0.05.
    Empirically (pre-rebuild): g=+0.78, p<0.001, n=30 paired seeds.
    The mean_jensen on adaptive (0.074) tracks DDQN (0.084), not
    vanilla (0.362), confirming the controller engages DDQN at the
    right batches.

    **AWAITING DATA (2026-05-12):** Postfix rebuild dropped the
    adaptive sweep corpora. Bridge returns POWER_INSUFFICIENT
    until they return."""
...
# =====================================================================
# RESTORED 2026-05-12 (audit reflection): the scope-limitation
# companion to the FourRooms recovery bridge. Same dormancy-aware
# controller FAILS on SpaceInvaders 1M (HELD g=−0.46, p=0.016 with
# n=30 historically). Together with the FourRooms HELD sibling,
# this pair encodes a non-trivial moderated claim: the dormancy
# invariant is NECESSARY but NOT SUFFICIENT — it correctly fires
# the controller on envs where DDQN helps, but fails to fire (or
# fires equally for both arms) on envs where DDQN hurts via a non-
# Hasselt channel. The pair is load-bearing for PAPER_NOTES.md
# §3.4's POWER_INSUFFICIENT-as-first-class-verdict architecture.
#
# Currently AWAITING DATA: `adaptive_dqn_spaceinvaders_1m` AND
# `minatar_1M_spaceinvaders` corpora absent from postfix cache.
# Bridge returns POWER_INSUFFICIENT until they return.
# =====================================================================
...
def adaptive_dqn_fails_to_avoid_attenuation__spaceinvaders_1m(
    ...
    from pure DDQN and inherits DDQN's late-burst attenuation.

    Auxiliary observation (empirical, paired n=30 pre-rebuild):
    ...
    (pre-rebuild): g=−0.46, p=0.016, n=30. INVARIANT_VIOLATION
    when adaptive unexpectedly RECOVERS the benefit (g ≥ +0.30,
    p<0.05) — would refute the dormancy-blind-spot reading.
    ...
    **AWAITING DATA (2026-05-12):** Postfix rebuild dropped both
    the adaptive sweep AND the SI 1M corpora. Bridge returns
...
# =====================================================================
# RESTORED 2026-05-12 (audit reflection): the FourRooms within-env
# do(γ) probe is the Pearl rung-2 evidence for CLAIM 5 — effective
# horizon moderates the chain-depth amplifier on FourRooms (HELD
# g=+1.11 at γ=0.99 historically). CLAIM 26b's three-gate panel
# covers FourRooms at γ=0.99 only — it does NOT test the WITHIN-ENV
# γ-amplification claim that this bridge encodes. Substance is not
# equivalently covered.
#
# Currently AWAITING DATA: `gamma_sweep` corpus is absent; current
# cache has only γ=0.99 on FourRooms (no within-env γ variation).
# Bridge returns POWER_INSUFFICIENT until a γ-sweep on FourRooms
# returns.
#
# Sister `ddqn_benefit_scales_with_effective_horizon__metamaze_high
# _gamma` STAYS CUT: its substantive claim (MetaMaze γ-amplification)
# IS tested on current data by `metamaze_link_steeper_at_high_gamma`
# (γ=0.99 + γ=0.999 in postfix corpora), which currently REFUTES
# the amplification. Restoring the OLD HELD verdict on missing data
# when the SAME claim is being actively refuted by present data
# would be inconsistent.
#
# Methodology note: bridge consumes paired_g + helped_fraction. The
# per-pair-Δ critique applies; future migration to arm_mean_diff
# deferred until data returns.
# =====================================================================
...
def ddqn_benefit_scales_with_effective_horizon__fourrooms(
    ...
    substantially positive there.

    The within-env γ-axis variation rules out env-confounders that
    bf-residual-on-g_link couldn't: bf ≈ 0.996 on every cell here,
    so any monotone relationship with effective_horizon must be
    γ-driven, hence intervention-causal under do(γ).
    ...
    HELD when helped_fraction ≥ 0.55 AND g ≥ 0.30 on the high-eff_h
    subset.

    **AWAITING DATA (2026-05-12):** Postfix rebuild dropped the
    `gamma_sweep` corpus. Bridge returns POWER_INSUFFICIENT until
    a γ-sweep on FourRooms returns. The within-env do(γ) probe is
    NOT covered by CLAIM 26b (which is cross-env at γ=0.99 only)."""
    if paired_g.n_pairs < 20:
        return Verdict.POWER_INSUFFICIENT
...
#
# Q-magnitude asymmetry by r_max sign (distinct from
# `env_reward_polarity`):
#   r_max > 0 envs: clip pulls Q DOWN; |Q| ↓.
#   r_max ≤ 0 envs: clip pulls Q toward less-positive (more
#                   negative); |Q| ↑.
# Note: `env_reward_polarity` is the framework's REACH/SURVIVAL
# measurable (Pearson r between episode length and episode
# return), which is a DISTINCT property — Acrobot is REACH
# (polarity≈-1) AND r_max=0; FourRooms is REACH (polarity≈-0.94)
# but r_max=+1. The clip-channel's sign-flip across the dormant
# panel tracks `env_reward_polarity`, not r_max sign; see
# CLAIM 3 docstring + `findings_clip_channel_polarity.md`.
#
...
def clip_wedge_predicts_outcome__polarity_moderated__dormant_scope(
    ...
    *,
    ...
    """Sufficient-condition complement to CLAIM 26b, with
    polarity-moderation folded into the predictor.

    Tests: on DORMANT cells where the Hasselt bias-correction
    channel is inactive (`jensen_gap < 0.1`), does the
    polarity-aligned clip wedge predict outcome benefit after
    partialling out residual `jensen_gap` variation?

    `clip_wedge_polarity_aligned = ddqn_bootstrap_gap × sign(
    env_reward_polarity)`. Folds the polarity moderation into a
    single predictor: if the clip channel's effect on outcome
    DEPENDS ON `env_reward_polarity` (positive on SURVIVAL,
    negative on REACH — as empirically observed pre-authoring on
    the dormant panel), the aligned predictor gives a unified
    positive-direction test. HELD here means "clip-wedge → outcome
    in polarity-consistent direction".

    **Predictor specifically (not r_max sign)**:
    `env_reward_polarity` is the framework's Pearson(episode_length,
    mc_return) measurable — the empirical REACH/SURVIVAL axis.
    Distinct from r_max sign: Acrobot is REACH (polarity=-0.99)
    with r_max=0; FourRooms is REACH (polarity=-0.94) with r_max=+1;
    both show the same channel-sign sign-flip (negative partial-r
    on dormant scope), which tracks polarity, not r_max.

    **Scope choices**:
    - `jensen_gap < 0.1` selects truly-dormant cells.
    - `|env_reward_polarity| > 0.3` drops ambiguous-polarity
      envs (Freeway nan, MetaMaze ~0). Including them is
      moot — `clip × sign(0)` is 0, so they'd just dilute.
    - Both arms enter scope; the per-env partial-r isn't arm-
      specific.

    **Verdict map**:
    - HELD when ρ_partial ≥ `min_rho` (polarity-moderated channel
      corroborated).
    - NO_EFFECT (SIGN_FLIP) when ρ_partial ≤ -`min_rho` — the
      polarity-alignment was the WRONG direction (e.g., clip
      hurts on SURVIVE AND helps on REACH, the opposite of the
      hypothesis).
    - POWER_INSUFFICIENT when |ρ_partial| < `min_rho` — channel
      is weak / data thin / hypothesis isn't the right shape.

    **Motivation** (per investigation 2026-05-12). On dormant
    cells: Asterix d_partial=+0.33, Breakout +0.25 (SURVIVAL,
    positive polarity); Acrobot -0.51, FourRooms -0.16 (REACH,
    negative polarity). Pooled partial-r with raw clip-wedge
    was +0.05 (washing out the polarity-conditional structure).
    The polarity-aligned predictor folds the interaction into a
    test the framework's existing primitives can handle.

    **What this bridge ISN'T**:
    - NOT a proof of causal mechanism — partial-r is rung-1.
      Polarity-conditional moderation could be confounded by
      env-structural factors (sparse-vs-dense reward density,
      avg episode length, etc.).
    - NOT a per-arm test — clip wedge is computed on both arms;
      within-arm-link primitive would refine "is the clip's
      predictive value the same in vanilla vs DDQN cells?".
    - NOT polarity-sign-direction-specific — for that, split
      into 3a/3b conditioning on polarity sign rather than
      folding into the predictor.

    **Pre-registration**: predicted_direction='a_gt_b'
    (positive partial-r ≥ 0.2). Falsification = sign-flip or
    spans-zero pooled across at least 4 polarity-defined
    strata (2 SURVIVAL + 2 REACH minimum). Authored
    2026-05-12 before final post-rebuild verdict landed."""
    del x, y, conditioning, stratify_by, min_stratum_size
    if stratified_partial_spearman.n_strata < min_strata:
        return Verdict.POWER_INSUFFICIENT
    rho = stratified_partial_spearman.rho_pooled
